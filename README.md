...
There are external libraries necessary to run the model. The only one necessary to run the model is PyTorch. However, if you wish to train the model, NumPy is required. And if you want to adjust the pre-training processes, specifically the data collection, BeautifulSoup is needed. Along with these libraries, git Large File Storage(LFS) is utilized, so when using the repository, make sure you have git LFS installed.
